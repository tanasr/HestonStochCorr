...
s0 = 2461.44; K = 2461.44; H = 1.5*s0; T = 12; M = 12
...
u_upper_bound = 70 # upper bound for u


# Formulas in brackets are from Hirsa page 63ff
def BarrierExtHestonUOC(s0,K,H,T,r,q,kv,mv,sv,v0,kr,mr,sr,rho0,rho2,alpha,M,u_upper_bound):
    # T = observation dates N
    Tvec = np.arange(0,T+1)/(T); dt = Tvec[1]-Tvec[0]; #M = len(Tvec)-1
    dk = 1
    x = log(s0/K); k = np.arange(0,u_upper_bound,dk); h = log(H/K); 
    # alpha_damping = 1.25
    ...
    if h<0:
        if alpha == 1:
            ...
            Vkm = np.zero(u_upper_bound) #because R = 0
        elif alpha == -1:
    ...
    u = np.arange(0,u_upper_bound)
    phi = np.zeros(len(u), dtype=complex)
    
    #main loop for up-and-out where x1=a, x2=h, c=h and d=b
    for m in range(M,1,-1): #in range M to 2 backwards with steps -1
        Ck = np.zeros(u_upper_bound) #define for each iteration an empty vector for Ck (2.59)
        for k in range(0,u_upper_bound):
            Mkj = Mkfunc(a,h,np.arange(0,u_upper_bound))
            
            # Solve extended heston phi for given u = range(0,u_upper_bound)
            # to obtain Ck as in (2.59) in Hirsa, using T = dt
            # Vkm is actually the option value at each monitoring point:
            char_fcn = lambda u: CF_OU(param,np.log(s0),r,dt,u)
            for idx, U in enumerate(u):
                phi[idx] = char_fcn( U ) # evaluate charfunc at each u
            Ck[k] = exp(-r*dt) * integrate.trapz( np.real(phi * Vkm * Mkj), x=u)
        Vkm = Ck 
    ...
    # minIdx is the integration lower bound k for the integration of the CDF, 
    # this is where the trapz integration starts. 
    # minIdx, = np.where((u >= np.log(s0)) == True)
    # int_index = minIdx[0]

    integrand = np.exp(-1j*u*np.log(s0)) * phi * Vkm
    # v = integrate.trapz( np.real(integrand[int_index:]), x=u[int_index:])
    v = integrate.trapz( np.real(integrand), x=u)
    v = 0.5 + 1 / pi * exp(-r*dt) * v
    return v 

t0 = time.time()
print(BarrierExtHestonUOC(s0,K,H,T,r,q,kv,mv,sv,v0,kr,mr,sr,rho0,rho2,alpha,M,u_upper_bound))
elapsed = time.time() - t0
print(elapsed)


"""
Results for: upper_bound =        60       70
value for L               L=12   35.23    35.23
                          L=14   40.0     40.50
                          L=20   47.90    47.90
                          L=40   30.66      xx
                          L=60   17.67      xx

CPU time between 4min and 6min
"""
